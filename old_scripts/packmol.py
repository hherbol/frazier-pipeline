...
def run(run_name, solvent_name, solute=None, seed=1):
	...
	if solute:
		solute = utils.Molecule('cml/'+solute, test_charges=False)
		system.add(solute)
...
write_restart '''+system.name+'''.restart''')
	...
	dielectrics = dict( zip(['THTO'], [42.84]) )
	extra_section='%%cosmo  SMD true  epsilon %f  end  %%basis aux auto NewECP Pb "def2-SD" "def2-TZVP" end NewGTO S "def2-TZVP" end end' % dielectrics[solvent_name]
	orca.job(run_name, '! OPT B97-D3 SV GCP(DFT/TZ) ECP{def2-TZVP} COSMO Grid3 FinalGrid5 SlowConv LooseOpt', system.atoms, queue='long',procs=16, mem=50, extra_section=extra_section, charge=2 if solute else 0)


#run(sys.argv[1], sys.argv[2], None if sys.argv[3]=='None' else sys.argv[3], int(sys.argv[4]) )

for i in [0,3,4,9]:
	for solvent in ['THTO']: #'methacrolein', 'gbl','ACN', 'DMF','dmso','nmp', 'acetone'
