...
def _get_enthalpy_solvation(solute, solvent, num_solvents=1, on_queue=False,
                            ...
                            charge_and_multiplicity_solute="0 1",
                            charge_and_multiplicity_solvent="0 1",
                            name_append="", route_lvls=[1, 1, 1, 1]):
    """
...
e_solv = fpl_auto.get_enthalpy_solvation("$SOLUTE",
    "$SOLVENT",
...
"""
        ...
        pysub_str = pysub_str.replace("$CHARGE_AND_MULTIPLICITY",
                                      charge_and_multiplicity)
        pysub_str = pysub_str.replace("$CHARGE_AND_MULTIPLICITY_SOLUTE",
                                      charge_and_multiplicity_solute)
        pysub_str = pysub_str.replace("$CHARGE_AND_MULTIPLICITY_SOLVENT",
                                      charge_and_multiplicity_solvent)
        ...
        def enthalpy(self):
            if not self.is_finished():
                ...
                H = open(self.name + ".log", "r").read()
                H = float(H.strip().split("\n")[-1])
                return H
    ...
    fpl_obj.extra_section = '''%basis aux auto
    NewECP Pb "def2-SD" "def2-TZVP" end
    NewECP Cs "def2-SD" "def2-TZVP" end
    NewGTO S "def2-TZVP" end
    end'''
    fpl_obj.charge_and_multiplicity = charge_and_multiplicity

    fpl_obj.route_solute = route_lvls[2]
    fpl_obj.extra_section_solute = '''%basis aux auto
    NewECP Pb "def2-SD" "def2-TZVP" end
    NewECP Cs "def2-SD" "def2-TZVP" end
    NewGTO S "def2-TZVP" end
    end'''
    ...
    fpl_obj.extra_section_solvent = '''%basis aux auto
    NewECP Pb "def2-SD" "def2-TZVP" end
    NewECP Cs "def2-SD" "def2-TZVP" end
    NewGTO S "def2-TZVP" end
    end'''
...
def get_mbo_given_criteria(MBOs, criteria, avg):
    ...
    if len(vals) == 0:
        raise Exception("No MBOs were found that matched your given criteria. \
This could be either \
(a) your criteria are too strict or (b) the simulation failed.")
    if avg:
...
"""
        ...
        if type(route_lvls) is list:
            ...
            r = str(route_lvls)
        job_name = fpl_utils.reduce_to_name(ion, halide, cation)
        job_name += "_" + solvent + "_" + r + ".py"
        fptr = open(job_name, "w")
        fptr.write(pysub_str)
        fptr.close()
        running_job = jobs.pysub(job_name,
        ...
        def mbo(self, criteria=[["O", "C"], ["O", "N"], ["O", "S"]], avg=True):
            if not self.is_finished():
                ...
                if not data.converged:
                    ...
                    mbo = fpl_auto.get_mbo_given_criteria(data.MBO,
                                                          criteria, avg)
                    return mbo
    ...
    task = lmp_large_job(fpl_obj,
                         run_name + "_large_lammps_" + str(route_lvls[0]))
    fpl_obj.add_task(task)
    ...
    #     2. Verify it is the same simulation in theory
    #     3. Read in this as the end state and don't re-run it (unless flag)
    task = lmp_small_job(fpl_obj,
                         run_name + "_small_lammps_" + str(route_lvls[0]))
    fpl_obj.add_task(task)
...
def get_UMBO(halide, cation, solvent,
    ...
    **Return**
    ...
    if on_queue:
        def umbo(self, offset=2.0,
                 criteria=[["O", "C"], ["O", "N"], ["O", "S"]], avg=True):
            if not self.is_finished():
                return None
            else:
                import orca
        ...
        if solvent in fpl_corrections.correct_solvent:
            return fpl_corrections.correct_solvent[solvent](halide,
                                                            cation,
                                                            ion)
        return vals
    else:
