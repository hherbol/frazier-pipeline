...
def job(run_name, prev_run_name, system, solvent_name, path=os.getcwd()+"/", debug=False,
	...
	if type(route) is int:
		...
		raise Exception("Currently all 1, 2 and 3 are the same (lvl 1, lowest)")
	...
	if implicit_solvent:
		route += " COSMO"
		extra_section = "%%cosmo SMD true epsilon " + dielectrics[solvent_name] + " end" + extra_section

	if not debug:
		orca.job(run_name, route, system.atoms, queue=dft_params["queue"], mem=dft_params["mem"], procs=dft_params["procs"], extra_section=extra_section, charge=dft_params["charge"])
